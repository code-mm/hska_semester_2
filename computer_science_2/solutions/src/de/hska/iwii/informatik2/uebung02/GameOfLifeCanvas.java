...
public class GameOfLifeCanvas extends Canvas {
  ...
  private Stop[] stops = new Stop[] { new Stop(0.0, GameOfLifeApplication.DARK_CELL_COLOR),
      new Stop(1.0, GameOfLifeApplication.BRIGHT_CELL_COLOR) };
  ...
  public void paint() {
    ...
    if (currentStartGeneration != null) {
      ...
      for (int x = 0; x < currentStartGeneration.length + 1; x++) {
        gc.strokeLine(x * cellWidth, 0, x * cellWidth, 
            currentStartGeneration[0].length * cellHeight);
      }
  ...
  /**
   * Canvas wird als ein in der Groesse unveraenderliches Bild betrachtet. Die
   * folgenden ueberschriebenen Methoden erlauben dem Layoutmanager eine
   * Groessenaenderung.
   * 
   * @return Immer <code>true</code>, weil die Groessenaenderung immer erlaubt
   *         ist.
   */
  @Override
